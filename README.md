...
> 安装脚本会改经 Go 模块镜像源获取 ipatool 源码与依赖并本地编译，全程不需要访问 GitHub。只要求本机装有 Go 1.25 或更高版本——可从官网 <https://go.dev/dl/> 下载安装包（中国大陆可用内容相同的镜像站 <https://golang.google.cn/dl/>，Windows 直接双击 `.msi` 安装），也可用 scoop / winget / brew 等包管理器安装。助手会先请你选择镜像源，实测可用的有：goproxy.cn（七牛云，默认）、goproxy.io、阿里云（mirrors.aliyun.com/goproxy/）、Go 官方（proxy.golang.org，中国大陆通常不可达），也可以用参数指定任意其他镜像。
